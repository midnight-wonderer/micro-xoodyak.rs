...
impl core::fmt::Write for UartWriter {
    fn write_str(&mut self, s: &str) -> core::fmt::Result {
        for byte in s.bytes() {
            unsafe {
                while (0x1000_0005 as *mut u8).read_volatile() & 0x20 == 0 {}
...
macro_rules! println {
    () => {
        let _ = core::fmt::write(&mut UartWriter, format_args!("\n"));
    };
    ($($arg:tt)*) => {
        let _ = core::fmt::write(&mut UartWriter, format_args!("{}\n", format_args!($($arg)*)));
    };
...
fn exit_qemu(success: bool) -> ! {
    unsafe {
        if success {
            ...
            (0x100000 as *mut u32).write_volatile(0x3333);
        }
    }
    loop {}
}
...
pub extern "C" fn main() -> ! {
    println!("=== RISC-V 32-bit Bare-metal Test Runner ===");

    test_keyed_empty();
    println!("test_keyed_empty: OK");

    test_unkeyed_empty();
    println!("test_unkeyed_empty: OK");

    test_encrypt();
    println!("test_encrypt: OK");

    test_unkeyed_hash();
    println!("test_unkeyed_hash: OK");

    test_aead();
    println!("test_aead: OK");

    test_aead_in_place();
    println!("test_aead_in_place: OK");

    test_aead_detached();
    println!("test_aead_detached: OK");
